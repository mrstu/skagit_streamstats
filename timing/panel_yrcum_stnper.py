...
#     figid="stn-%s"%station    
    ...
    pancomb = [(station,'rcp45','bc'), (station,'rcp85','bc')]    
    pannumrc='24' #rows, columns
    figsize=(10,6)    
    figid="stn-%s-%s"%(station,'rcp4585-bc')

#     pancomb = [(station,'rcp45','nobc'), (station,'rcp85','nobc')]    
#     pannumrc='24' #rows, columns
#     figsize=(10,6)    
#     figid="stn-%s-%s"%(station,'rcp4585-nobc')

#     pancomb = [(station,'rcp85','nobc'), (station,'rcp85','bc')]    
#     pannumrc='24' #rows, columns
#     figsize=(10,6)    
#     figid="stn-%s-%s"%(station,'rcp85-bcnobc')
...
#             for igcm, gcm in enumerate(gcms[:2]):                
            ...
            #     lcolors = ['k']
                ...
                for nper, period in enumerate(periods):
                    ...
                    ax = fig.add_subplot('%s%d'%(pannumrc,nper+1+npan*4), polar=True)                        
...
#                     ax.plot(itheta, irad, color=lcolors[nper], alpha=0.75) #, linewidth=4)        
                    if nper == 0 and npan == 0:
                        lh, = ax.plot(itheta, irad, alpha=0.75, linewidth=1, color=colormap(cparts[igcm]), clip_on=False)
                        handles.append(lh)
                    else:
                        ax.plot(itheta, irad, alpha=0.75, linewidth=1, color=colormap(cparts[igcm]), clip_on=False)
                    if nper == 0:
                        plt.ylabel(', '.join(ipan), **{'labelpad':85})                        
                        #axes.yaxis.labelpad                        
...
#     for igcm, gcm in enumerate(gcms):
    plt.figlegend(handles, gcms, 'lower center', ncol=5) #ax.plot(itheta, irad, alpha=0.75, linewidth=1, color=colormap(cparts[igcm]))        
#                 plt.title('%s, %s\n%s, %s\n'%(stnname,treatment,gcm,scen)+r'$Q_{10,hist} = %5.2f$ '%dft.loc[istn].mean()+r'$[m^{3}s^{-1}]$')                            
#     plt.suptitle('Normalized Annual Peak Flow by Epoch ['+r'$Q_{peak}/Q_{10,hist}$'+']')            
#     plt.suptitle('Normalized Annual Peak Flow by Epoch ['+r'$Q_{peak}/Q_{10,hist}$'+']\n'+'%s, %s, %s, '%(stnname,treatment,scen)+r'$Q_{10,hist} = %5.2f$ '%dft.loc[istn].mean()+r'$[m^{3}s^{-1}]$')
    if dorecint:
        if args[5] == 'ensemble':
#             oname = os.path.join(odir, 'tcum_r-%s_%s_%s_%s_%03.1F.svg'%(recint, scen, args[5], treatment, rlim))
            oname = os.path.join(odir, 'tcum-%s_r-%s_%s_%03.1F'%(figid, recint, args[5], rlim))            
            print 'saving ensemble'
        else:
            oname = os.path.join(odir, 'tcum-%s_r-%s_%s_%03.1F'%(figid, recint, args[6], rlim))
#     else:            
